...
which is the expectation of $p(\theta_j | X, \mu, \eta)$ over the posteriore $p(\mu,\eta | X)$. Computing posterior distributions
like this is sometimes referred to as
"integrating out uncertainty in" $\mu,\eta$. (It is useful to compare
...
Using the same idea, given an expression to approximate the posterior second moment $E(\theta^2_j | X)$, and so approximate the posterior variance
